...
> 在当前 RK3588 / OpenHarmony 板子上，官方重编后的 `hcodec_demo_arm(apiType=1, buffer)` 已经证明：buffer 硬解链路不仅吞吐高于 CPU 软解，而且 CPU 占用只有软解的一个零头。这个优势在 `1080p60` 的 `10s / 30s / 60s / 10m18s` 和 `2688x1520@60` 的 `10s / 30s / 60s` 上都保持稳定。对后续“buffer decode -> YOLO inference”来说，这意味着硬解并不只是更快，而是明显更省 CPU，更适合作为后续推理前端。
